...
public class Main {
    ...
    public int maxArgsStm(Stm.T stm) {
        if (stm instanceof Stm.Compound) {
            ...
            return new Todo().maxArgsAssign((Stm.Assign) stm);
        } else if (stm instanceof Stm.Print) {
            return new Todo().maxArgsPrint((Stm.Print) stm);
        } else
            new Bug();
        return 0;
    }
    ...
    private void interpStm(Stm.T prog) {
        if (prog instanceof Stm.Compound) {
            new Todo().interpCompound((Stm.Compound) prog);
        } else if (prog instanceof Stm.Assign) {
            new Todo().interpAssign((Stm.Assign) prog);
        } else if (prog instanceof Stm.Print) {
            new Todo().interpPrint((Stm.Print) prog);
        } else
            new Bug();
    }
    ...
    public static void main(String[] args) {
        new Main().interpStm(Samples.prog);
    }
}
